...
interface GoogleAdsKeywordsForKeywordsKeyword {
  keyword: string;
...
export default function GoogleAdsKeywordsForKeywordsPage() {
  ...
  const [results, setResults] = useState<GoogleAdsKeywordsForKeywordsKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: GoogleAdsKeywordsForKeywordsKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=google_ads_keywords_for_keywords&limit=10');
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/google-ads-keywords-for-keywords', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=google_ads_keywords_for_keywords&limit=10');
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Keyword', 'Search Volume', 'Competition Index', 'Competition', 'CPC', 'Low Top Bid', 'High Top Bid'];
    const csvContent = [
      headers.join(','),
      ...results.map(result => [
        `"${result.keyword}"`,
    ...
    link.setAttribute('download', `google-ads-keywords-for-keywords-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Google Ads Keywords for Keywords</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Generiere verwandte Keyword-Ideen basierend auf Google Ads Daten. Maximal 20 Keywords pro Anfrage, maximal 80 Zeichen pro Keyword.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Google Ads Keywords for Keywords Analyse</CardTitle>
          <CardDescription>
            Finde verwandte Keywords basierend auf Google Ads Suchvolumen und Wettbewerbsdaten.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="keywords">Keywords (ein Keyword pro Zeile, max 20 Keywords, max 80 Zeichen)</Label>
              <textarea
                id="keywords"
                value={keywords}
