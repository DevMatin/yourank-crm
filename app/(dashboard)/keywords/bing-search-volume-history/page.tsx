...
interface BingSearchVolumeHistoryKeyword {
  keyword: string;
...
export default function BingSearchVolumeHistoryPage() {
  ...
  const [results, setResults] = useState<BingSearchVolumeHistoryKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: BingSearchVolumeHistoryKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=bing_search_volume_history&limit=10');
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/bing-search-volume-history', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=bing_search_volume_history&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `bing-search-volume-history-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Bing Search Volume History</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Erhalte historische Suchvolumen-Daten für Bing mit monatlichen Zeitreihen. Maximal 1000 Keywords pro Anfrage, maximal 100 Zeichen pro Keyword.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Bing Search Volume History Analyse</CardTitle>
          <CardDescription>
            Analysiere historische Suchvolumen-Trends für Bing Keywords mit detaillierten monatlichen Daten.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="keywords">Keywords (ein Keyword pro Zeile, max 1000 Keywords, max 100 Zeichen)</Label>
              <textarea
                id="keywords"
                value={keywords}
                ...
                <Card key={index}>
                  ...
                  <CardContent>
                    ...
                    {result.monthly_searches && result.monthly_searches.length > 0 && (
                      <div className="mt-4">
                        <div className="font-medium text-muted-foreground mb-2">Monatliche Suchen (Historische Daten)</div>
