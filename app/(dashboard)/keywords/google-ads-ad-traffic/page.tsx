...
interface GoogleAdsAdTrafficKeyword {
  keyword: string;
...
export default function GoogleAdsAdTrafficPage() {
  ...
  const [results, setResults] = useState<GoogleAdsAdTrafficKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: GoogleAdsAdTrafficKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=google_ads_ad_traffic&limit=10');
        if (response.ok) {
          ...
          const historyData = data.analyses?.map((analysis: any) => ({
            ...
            match: analysis.input?.match || analysis.parameters?.match || 'exact',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!bidValue || bidValue <= 0) {
        throw new Error('Bid muss eine positive Zahl sein');
      }
      
      const response = await fetch('/api/keywords/google-ads-ad-traffic', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=google_ads_ad_traffic&limit=10');
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Keyword', 'Search Volume', 'Competition Index', 'Competition', 'CPC', 'Ad Position', 'Cost', 'Clicks', 'Impressions', 'CTR'];
    const csvContent = [
      headers.join(','),
      ...results.map(result => [
        `"${result.keyword}"`,
    ...
    link.setAttribute('download', `google-ads-ad-traffic-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Google Ads Ad Traffic</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Erhalte Traffic-Schätzungen für Keywords basierend auf Gebotspreis und Match-Type. Maximal 1000 Keywords pro Anfrage.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Google Ads Ad Traffic Analyse</CardTitle>
          <CardDescription>
            Analysiere Traffic-Schätzungen für Keywords basierend auf deinem Gebotspreis und Match-Type.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="keywords">Keywords (ein Keyword pro Zeile, max 1000 Keywords)</Label>
              <textarea
                id="keywords"
                value={keywords}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="bid">Gebotspreis (USD)</Label>
                <Input
                  id="bid"
                  type="number"
                  step="0.01"
              ...
              <div className="space-y-2">
                ...
                <Select value={match} onValueChange={setMatch}>
                  <SelectTrigger>
                    <SelectValue placeholder="Match Type auswählen" />
                  </SelectTrigger>
                  <SelectContent>
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            />
            <KeywordMetricsCard
              title="Durchschnittliche Klicks"
              value={Math.round(results.reduce((sum, r) => sum + (r.clicks || 0), 0) / results.length).toLocaleString()}
              icon={Target}
            />
            <KeywordMetricsCard
              title="Durchschnittliche Kosten"
              value={`$${((results.reduce((sum, r) => sum + (r.cost || 0), 0) / results.length)).toFixed(2)}`}
              icon={TrendingUp}
            />
          </div>
                ...
                <Card key={index}>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        <div className="font-medium text-muted-foreground">Klicks</div>
                        <div className="text-lg font-semibold">
                          {result.clicks?.toLocaleString() || 'N/A'}
                        </div>
                      </div>
                      <div>
                        <div className="font-medium text-muted-foreground">Kosten</div>
                        <div className="text-lg font-semibold">
                          {result.cost ? `$${result.cost.toFixed(2)}` : 'N/A'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mt-4">
                      <div>
                        <div className="font-medium text-muted-foreground">Ad Position</div>
                        <div className="text-lg font-semibold">
                          {result.ad_position || 'N/A'}
                        </div>
                      </div>
                      <div>
                        <div className="font-medium text-muted-foreground">Impressions</div>
                        <div className="text-lg font-semibold">
                          {result.impressions?.toLocaleString() || 'N/A'}
                        </div>
                      </div>
                      <div>
                        <div className="font-medium text-muted-foreground">CTR</div>
                        <div className="text-lg font-semibold">
                          {result.ctr ? `${(result.ctr * 100).toFixed(2)}%` : 'N/A'}
                        </div>
                      </div>
                      <div>
