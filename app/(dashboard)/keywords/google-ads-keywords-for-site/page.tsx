...
export default function GoogleAdsKeywordsForSitePage() {
  ...
  const [results, setResults] = useState<GoogleAdsKeyword[]>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=google_ads_keywords_for_site&limit=10');
        if (response.ok) {
          const data = await response.json();
          const historyData = data.analyses?.map((analysis: any) => ({
            target: analysis.input?.target || analysis.parameters?.target || 'Unbekannte Website',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/keywords/google-ads-keywords-for-site', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=google_ads_keywords_for_site&limit=10');
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Keyword', 'Search Volume', 'Competition', 'CPC', 'Low Top Bid', 'High Top Bid'];
    const csvContent = [
      headers.join(','),
      ...results.map(result => [
        `"${result.keyword}"`,
        ...
        result.cpc || 0,
        result.low_top_of_page_bid || 0,
        result.high_top_of_page_bid || 0
      ].join(','))
    ].join('\n');
    ...
    link.setAttribute('download', `google-ads-keywords-${target.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Google Ads Keywords for Site</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Finde Keywords, die für eine bestimmte Website relevant sind, basierend auf Google Ads Daten und Suchvolumen.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Website Keyword-Analyse</CardTitle>
          <CardDescription>
            Analysiere welche Keywords für eine bestimmte Website relevant sind und erhalte detaillierte Metriken.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Button type="submit" disabled={loading} className="w-full">
              {loading ? (
                <>
                  ...
                  Analysiere Website Keywords...
                </>
              ) : (
                <>
                  <Search className="mr-2 h-4 w-4" />
                  Website Keywords analysieren
                </>
              )}
            </Button>
          </form>
      ...
      {loadingHistory ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Deine letzten Website Keyword-Analysen
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="mt-1 text-sm text-muted-foreground">
                        {historyItem.resultsCount} Keywords gefunden
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Button
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Führe deine erste Domain-Analyse durch, um hier Ergebnisse zu sehen.</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-semibold">Website Keywords ({results.length} gefunden)</h2>
                ...
                <Card key={index}>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        <div className="font-medium text-muted-foreground">Low Top Bid</div>
                        <div className="text-lg font-semibold">
                          {result.low_top_of_page_bid ? `$${result.low_top_of_page_bid.toFixed(2)}` : 'N/A'}
                        </div>
                      </div>
                    </div>
