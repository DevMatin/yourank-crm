...
import { KeywordsDataDataforseoTrendsSubregionInterestsLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate DataForSEO Trends Subregion Interests limits
    if (keywords.length > 5) {
      return NextResponse.json(
        { error: 'Maximum 5 keywords allowed for DataForSEO Trends Subregion Interests' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const request = new KeywordsDataDataforseoTrendsSubregionInterestsLiveRequestInfo();
      ...
      logger.info('Sending DataForSEO Trends Subregion Interests request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.dataforseoTrendsSubregionInterestsLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No DataForSEO Trends Subregion Interests data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No DataForSEO Trends Subregion Interests data found for these keywords',
      ...
      logger.info('✅ DataForSEO data verified - contains valid DataForSEO Trends Subregion Interests structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing DataForSEO Trends Subregion Interests item:', JSON.stringify(item, null, 2));
        
        return {
          keyword: item.keyword || 'Unknown',
          subregion_data: item.subregion_data || [],
          interests_map: item.interests_map || [],
          regional_interests: item.regional_interests || {},
          keyword_info: {
            se_type: item.se_type || 'dataforseo_trends_subregion_interests',
      ...
      // Debug logging
      logger.info('Processed DataForSEO Trends Subregion Interests Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'dataforseoTrendsSubregionInterestsLive',
        timestamp: new Date().toISOString(),
        dataForSeoTaskId: result.tasks[0].id
      });

    } catch (apiError) {
      logger.error('DataForSEO Trends Subregion Interests API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der DataForSEO Trends Subregion Interests Daten. Bitte versuche es erneut.';
    ...
    logger.error('DataForSEO Trends Subregion Interests API Error:', error);
