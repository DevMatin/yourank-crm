...
import { KeywordsDataGoogleAdsKeywordsForSiteLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { target, target_type = 'page', location = 'Germany', language = 'German', date_from, date_to, limit = 100, search_partners = false, include_adult_keywords = false, sort_by = 'relevance' } = body;
    ...
    // Check credits
    const requiredCredits = 5; // Higher cost for site analysis
    await checkUserCredits(user.id, requiredCredits);

    // Save analysis record
    const analysisRecord = await saveAnalysis(
      { target, target_type, location, language, date_from, date_to, limit, search_partners, include_adult_keywords, sort_by },
    ...
    try {
      ...
      logger.info('Sending Google Ads Keywords for Site request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.googleAdsKeywordsForSiteLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No Google Ads keywords for site data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No Google Ads keywords found for this website',
      ...
      logger.info('✅ DataForSEO data verified - contains valid Google Ads keywords for site structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing Google Ads Keywords for Site item:', JSON.stringify(item, null, 2));
      ...
      // Debug logging
      logger.info('Processed Google Ads Keywords for Site Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'googleAdsKeywordsForSiteLive',
      ...
      logger.error('DataForSEO Google Ads Keywords for Site API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der Google Ads Keywords für diese Website. Bitte versuche es erneut.';
    ...
    logger.error('Google Ads Keywords for Site API Error:', error);
