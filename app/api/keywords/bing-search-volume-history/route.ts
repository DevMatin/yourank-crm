...
import { KeywordsDataBingSearchVolumeHistoryLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate Bing Search Volume History limits
    if (keywords.length > 1000) {
      return NextResponse.json(
        { error: 'Maximum 1000 keywords allowed for Bing Search Volume History' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const request = new KeywordsDataBingSearchVolumeHistoryLiveRequestInfo();
      ...
      logger.info('Sending Bing Search Volume History request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.bingSearchVolumeHistoryLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No Bing Search Volume History data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No Bing Search Volume History data found for these keywords',
      ...
      logger.info('✅ DataForSEO data verified - contains valid Bing Search Volume History structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing Bing Search Volume History item:', JSON.stringify(item, null, 2));
        ...
        return {
          ...
          keyword_info: {
            se_type: item.se_type || 'bing_search_volume_history',
      ...
      // Debug logging
      logger.info('Processed Bing Search Volume History Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'bingSearchVolumeHistoryLive',
        timestamp: new Date().toISOString(),
        dataForSeoTaskId: result.tasks[0].id
      });

    } catch (apiError) {
      logger.error('DataForSEO Bing Search Volume History API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der Bing Search Volume History Daten. Bitte versuche es erneut.';
    ...
    logger.error('Bing Search Volume History API Error:', error);
