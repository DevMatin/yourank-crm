...
import { KeywordsDataClickstreamDataDataforseoSearchVolumeLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate Clickstream DataForSEO Search Volume limits
    if (keywords.length > 1000) {
      return NextResponse.json(
        { error: 'Maximum 1000 keywords allowed for Clickstream DataForSEO Search Volume' },
        { status: 400 }
      );
    }
    ...
    const analysisRecord = await saveAnalysis(
      { keywords, location, language, use_clickstream },
    ...
    try {
      ...
      const request = new KeywordsDataClickstreamDataDataforseoSearchVolumeLiveRequestInfo();
      request.keywords = keywords.map(k => k.trim());
      request.location_name = location;
      request.language_name = language;
      request.use_clickstream = use_clickstream;

      logger.info('Sending Clickstream DataForSEO Search Volume request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.clickstreamDataDataforseoSearchVolumeLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No Clickstream DataForSEO Search Volume data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No Clickstream DataForSEO Search Volume data found for these keywords',
      ...
      logger.info('✅ DataForSEO data verified - contains valid Clickstream DataForSEO Search Volume structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing Clickstream DataForSEO Search Volume item:', JSON.stringify(item, null, 2));
        ...
        return {
          ...
          keyword_info: {
            se_type: item.se_type || 'clickstream_dataforseo_search_volume',
      ...
      // Debug logging
      logger.info('Processed Clickstream DataForSEO Search Volume Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'clickstreamDataDataforseoSearchVolumeLive',
        timestamp: new Date().toISOString(),
        dataForSeoTaskId: result.tasks[0].id
      });

    } catch (apiError) {
      logger.error('DataForSEO Clickstream DataForSEO Search Volume API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der Clickstream DataForSEO Search Volume Daten. Bitte versuche es erneut.';
    ...
    logger.error('Clickstream DataForSEO Search Volume API Error:', error);
