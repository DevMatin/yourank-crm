...
import { KeywordsDataClickstreamDataGlobalSearchVolumeLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { keywords, location = 'Germany', language = 'German', device = 'all', date_from, date_to } = body;
    ...
    // Validate Clickstream limits
    if (keywords.length > 1000) {
      return NextResponse.json(
        { error: 'Maximum 1000 keywords allowed for Clickstream Global Search Volume' },
        { status: 400 }
      );
    }

    // Validate keyword length
    const invalidKeywords = keywords.filter(k => k.trim().length < 3);
    ...
    // Check credits
    const requiredCredits = keywords.length * 2; // 2 credits per keyword for clickstream data
    await checkUserCredits(user.id, requiredCredits);

    // Save analysis record
    const analysisRecord = await saveAnalysis(
      { keywords, location, language, device, date_from, date_to },
    ...
    try {
      ...
      const request = new KeywordsDataClickstreamDataGlobalSearchVolumeLiveRequestInfo();
      ...
      logger.info('Sending Clickstream Global Search Volume request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.clickstreamDataGlobalSearchVolumeLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No Clickstream global search volume data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No Clickstream global search volume data found for these keywords',
      ...
      logger.info('✅ DataForSEO data verified - contains valid Clickstream global search volume structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing Clickstream item:', JSON.stringify(item, null, 2));
        ...
        return {
          ...
          clickstream_data: item.clickstream_data || [],
          keyword_info: {
            se_type: item.se_type || 'clickstream',
            ...
            monthly_searches: item.monthly_searches || [],
            device: device,
            data_source: 'clickstream_global'
          }
        };
      });

      // Debug logging
      logger.info('Processed Clickstream Global Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'clickstreamDataGlobalSearchVolumeLive',
        timestamp: new Date().toISOString(),
        dataForSeoTaskId: result.tasks[0].id
      });

    } catch (apiError) {
      logger.error('DataForSEO Clickstream Global API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der Clickstream Global Suchvolumen-Daten. Bitte versuche es erneut.';
    ...
    logger.error('Clickstream Global Search Volume API Error:', error);
