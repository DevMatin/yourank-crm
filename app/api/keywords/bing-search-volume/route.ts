...
import { KeywordsDataBingSearchVolumeLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { keywords, location = 'Germany', language = 'German', device = 'all', date_from, date_to, search_partners = false, sort_by = 'relevance' } = body;
    ...
    // Validate Bing Search Volume limits
    if (keywords.length > 1000) {
      return NextResponse.json(
        { error: 'Maximum 1000 keywords allowed for Bing Search Volume' },
        { status: 400 }
      );
    }

    // Validate keyword length
    const invalidKeywords = keywords.filter(k => k.trim().length > 100);
    ...
    // Check credits
    const requiredCredits = keywords.length * 1; // 1 credit per keyword for Bing
    await checkUserCredits(user.id, requiredCredits);

    // Save analysis record
    const analysisRecord = await saveAnalysis(
      { keywords, location, language, device, date_from, date_to, search_partners, sort_by },
      'bing_search_volume',
      user.id,
      undefined,
      requiredCredits
    ...
    try {
      ...
      const request = new KeywordsDataBingSearchVolumeLiveRequestInfo();
      ...
      logger.info('Sending Bing Search Volume request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.bingSearchVolumeLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No Bing search volume data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No Bing search volume data found for these keywords',
      ...
      logger.info('✅ DataForSEO data verified - contains valid Bing search volume structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing Bing item:', JSON.stringify(item, null, 2));
      ...
      // Debug logging
      logger.info('Processed Bing Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'bingSearchVolumeLive',
        timestamp: new Date().toISOString(),
        dataForSeoTaskId: result.tasks[0].id
      });

    } catch (apiError) {
      logger.error('DataForSEO Bing API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der Bing Suchvolumen-Daten. Bitte versuche es erneut.';
    ...
    logger.error('Bing Search Volume API Error:', error);
