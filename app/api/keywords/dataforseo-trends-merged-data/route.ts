...
import { KeywordsDataDataforseoTrendsMergedDataLiveRequestInfo } from 'dataforseo-client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate DataForSEO Trends Merged Data limits
    if (keywords.length > 5) {
      return NextResponse.json(
        { error: 'Maximum 5 keywords allowed for DataForSEO Trends Merged Data' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const request = new KeywordsDataDataforseoTrendsMergedDataLiveRequestInfo();
      ...
      logger.info('Sending DataForSEO Trends Merged Data request to DataForSEO:', {
      ...
      while (retryCount < maxRetries) {
        try {
          result = await dataForSeoClient.keywords.dataforseoTrendsMergedDataLive([request]);
      ...
      if (!result?.tasks?.[0]?.result?.[0]?.items || result.tasks[0].result[0].items.length === 0) {
        logger.info('No DataForSEO Trends Merged Data found in DataForSEO response');
        ...
        return NextResponse.json({
          ...
          creditsUsed: 0,
          message: 'No DataForSEO Trends Merged Data found for these keywords',
      ...
      logger.info('✅ DataForSEO data verified - contains valid DataForSEO Trends Merged Data structure');

      // Process results
      const processedResults = result.tasks[0].result[0].items.map((item: any) => {
        logger.info('Processing DataForSEO Trends Merged Data item:', JSON.stringify(item, null, 2));
        
        return {
          keyword: item.keyword || 'Unknown',
          trend_data: item.trend_data || [],
          data_sources: item.data_sources || [],
          merged_metrics: item.merged_metrics || {},
          keyword_info: {
            se_type: item.se_type || 'dataforseo_trends_merged_data',
      ...
      // Debug logging
      logger.info('Processed DataForSEO Trends Merged Data Results:', JSON.stringify(processedResults, null, 2));
      ...
      return NextResponse.json({
        ...
        source: 'DataForSEO API',
        apiEndpoint: 'dataforseoTrendsMergedDataLive',
        timestamp: new Date().toISOString(),
        dataForSeoTaskId: result.tasks[0].id
      });

    } catch (apiError) {
      logger.error('DataForSEO Trends Merged Data API Error:', apiError);
      ...
      let errorMessage = 'Fehler beim Abrufen der DataForSEO Trends Merged Data. Bitte versuche es erneut.';
    ...
    logger.error('DataForSEO Trends Merged Data API Error:', error);
