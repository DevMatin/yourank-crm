...
interface BingKeywordPerformanceKeyword {
  keyword: string;
...
export default function BingKeywordPerformancePage() {
  const [keywords, setKeywords] = useState('');
  const [location, setLocation] = useState('Germany');
  const [language, setLanguage] = useState('German');
  const [device, setDevice] = useState('all');
  const [match, setMatch] = useState('aggregate');
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<BingKeywordPerformanceKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: BingKeywordPerformanceKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=bing_keyword_performance&limit=10');
        if (response.ok) {
          ...
          const historyData = data.analyses?.map((analysis: any) => ({
            ...
            device: analysis.input?.device || analysis.parameters?.device || 'all',
            match: analysis.input?.match || analysis.parameters?.match || 'aggregate',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/bing-keyword-performance', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=bing_keyword_performance&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `bing-keyword-performance-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Bing Keyword Performance</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Analysiere Performance-Metriken für Bing Keywords mit Device- und Match-Type Filtern. Maximal 1000 Keywords pro Anfrage.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Bing Keyword Performance Analyse</CardTitle>
          <CardDescription>
            Erhalte detaillierte Performance-Metriken für Bing Keywords mit verschiedenen Device- und Match-Type Filtern.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={match} onValueChange={setMatch}>
                  ...
                  <SelectContent>
                    <SelectItem value="aggregate">Aggregate (Standard)</SelectItem>
