...
export default function BingKeywordsForSitePage() {
  ...
  const [results, setResults] = useState<BingKeyword[]>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=bing_keywords_for_site&limit=10');
        if (response.ok) {
          const data = await response.json();
          if (data.success && data.analyses) {
            const historyData = data.analyses.map((analysis: any) => ({
              target: analysis.input?.target || 'Unbekannte Website',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/keywords/bing-keywords-for-site', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=bing_keywords_for_site&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `bing-keywords-${target.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Bing Keywords for Site</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Finde Keywords, die für eine bestimmte Website relevant sind, basierend auf Bing Suchdaten und Suchvolumen.
      </p>
          ...
          <CardContent>
            ...
            <CardContent>
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Führe deine erste Website Keyword-Analyse durch, um hier Ergebnisse zu sehen.</p>
              </div>
            </CardContent>
          )}
        </Card>
          ...
          {viewMode === 'cards' ? (
            ...
            <RelatedKeywordsTable 
              data={results.map(result => ({
                ...
                competition: 0, // Bing doesn't have competition index like Google
                cpc: result.cpc || 0,
                trend: 0,
                related_keywords: []
              }))}
