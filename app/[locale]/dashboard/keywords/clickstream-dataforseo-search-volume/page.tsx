...
interface ClickstreamDataforseoSearchVolumeKeyword {
  keyword: string;
...
export default function ClickstreamDataforseoSearchVolumePage() {
  const [keywords, setKeywords] = useState('');
  const [location, setLocation] = useState('Germany');
  const [language, setLanguage] = useState('German');
  const [useClickstream, setUseClickstream] = useState(true);
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<ClickstreamDataforseoSearchVolumeKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: ClickstreamDataforseoSearchVolumeKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=clickstream_dataforseo_search_volume&limit=10');
        if (response.ok) {
          ...
          const historyData = data.analyses?.map((analysis: any) => ({
            ...
            useClickstream: analysis.input?.use_clickstream || analysis.parameters?.use_clickstream || true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/clickstream-dataforseo-search-volume', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=clickstream_dataforseo_search_volume&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `clickstream-dataforseo-search-volume-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const loadHistoryResult = (historyItem: any) => {
    ...
    setUseClickstream(historyItem.useClickstream);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Clickstream DataForSEO Search Volume</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Erhalte DataForSEO-eigene Suchvolumen-Berechnung basierend auf Clickstream-Daten. Maximal 1000 Keywords pro Anfrage, mindestens 3 Zeichen pro Keyword.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Clickstream DataForSEO Search Volume Analyse</CardTitle>
          <CardDescription>
            Analysiere Suchvolumen mit DataForSEO-eigener Berechnung basierend auf Clickstream-Daten oder Bing-Daten.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="useClickstream">Clickstream-Daten verwenden</Label>
              <div className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  id="useClickstream"
                  checked={useClickstream}
                  onChange={(e) => setUseClickstream(e.target.checked)}
                  className="rounded border-gray-300"
                />
                <Label htmlFor="useClickstream" className="text-sm text-muted-foreground">
                  {useClickstream 
                    ? 'DataForSEO Suchvolumen basierend auf Clickstream-Daten' 
                    : 'Bing Suchvolumen-Daten für DataForSEO Berechnung'
                  }
                </Label>
              </div>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <Badge variant={historyItem.useClickstream ? "default" : "outline"} className="text-xs">
                          {historyItem.useClickstream ? 'Clickstream' : 'Bing'}
                        </Badge>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Badge variant={useClickstream ? "default" : "outline"} className="text-sm">
                {useClickstream ? 'Clickstream' : 'Bing'}
              </Badge>
            </div>
          </div>
