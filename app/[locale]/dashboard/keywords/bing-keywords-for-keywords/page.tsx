...
interface BingKeywordsForKeywordsKeyword {
  keyword: string;
...
export default function BingKeywordsForKeywordsPage() {
  ...
  const [results, setResults] = useState<BingKeywordsForKeywordsKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: BingKeywordsForKeywordsKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=bing_keywords_for_keywords&limit=10');
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/bing-keywords-for-keywords', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=bing_keywords_for_keywords&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `bing-keywords-for-keywords-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Bing Keywords for Keywords</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Generiere Bing-spezifische Keyword-Vorschläge mit Device-Filter. Maximal 200 Keywords pro Anfrage, maximal 100 Zeichen pro Keyword.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Bing Keywords for Keywords Analyse</CardTitle>
          <CardDescription>
            Finde verwandte Keywords basierend auf Bing Suchvolumen und Wettbewerbsdaten.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="keywords">Keywords (ein Keyword pro Zeile, max 200 Keywords, max 100 Zeichen)</Label>
              <textarea
                id="keywords"
                value={keywords}
              ...
              <div className="space-y-2">
                ...
                <Select value={sortBy} onValueChange={setSortBy}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="search_volume">Suchvolumen</SelectItem>
