...
interface BingSearchVolumeKeyword {
  keyword: string;
...
export default function BingSearchVolumePage() {
  const { t, tCommon, pageTitle, searchPlaceholder, analyzeButton, locationLabel, languageLabel, loadingText, creditsText, resultsText } = useKeywordsPageTranslations('bing-search-volume');
  ...
  const [results, setResults] = useState<BingSearchVolumeKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: BingSearchVolumeKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=bing_search_volume&limit=10');
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/bing-search-volume', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=bing_search_volume&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `bing-search-volume-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('searchVolume')}</CardTitle>
          <CardDescription>
            {t('subtitle')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="keywords">{tCommon('keywords')} ({tCommon('onePerLine')})</Label>
              <textarea
                id="keywords"
                value={keywords}
