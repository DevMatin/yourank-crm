...
interface GoogleAdsSearchVolumeKeyword {
  keyword: string;
...
export default function GoogleAdsSearchVolumePage() {
  ...
  const [results, setResults] = useState<GoogleAdsSearchVolumeKeyword[]>([]);
  ...
  const [searchHistory, setSearchHistory] = useState<Array<{
    ...
    timestamp: string;
    resultsCount: number;
    results: GoogleAdsSearchVolumeKeyword[];
    analysisId: string;
  }>>([]);
  ...
  useEffect(() => {
    const loadSearchHistory = async () => {
      try {
        const response = await fetch('/api/analysis/history?type=google_ads_search_volume&limit=10');
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/keywords/google-ads-search-volume', {
        method: 'POST',
        headers: {
      ...
      const historyResponse = await fetch('/api/analysis/history?type=google_ads_search_volume&limit=10');
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `google-ads-search-volume-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Google Ads Search Volume</h1>
        </div>
        {results.length > 0 && (
          <div className="flex items-center space-x-2">
            <Button
      ...
      <p className="text-muted-foreground">
        Erhalte präzise Google Ads Suchvolumen-Daten mit erweiterten Metriken wie Competition Index, CPC und Top-of-Page Bids.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>Google Ads Suchvolumen-Analyse</CardTitle>
          <CardDescription>
            Analysiere das Suchvolumen und die Wettbewerbsintensität deiner Keywords basierend auf Google Ads Daten.
          </CardDescription>
        </CardHeader>
        <CardContent>
