...
import { Play, Search, Star, Download, TrendingUp } from 'lucide-react';
...
export default function PlayStorePage() {
  ...
  const [results, setResults] = useState<PlayStoreResult | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/appdata/play-store', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || 'Fehler bei der Play Store Analyse');
      }

      const data = await response.json();
      setResults(data.result);
      
      toast.success('Play Store Analyse erfolgreich abgeschlossen!');
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Play Store Analysis</h1>
        <p className="text-muted-foreground">
          Analysiere Google Play Store Rankings und Daten
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  id="app_name"
                  placeholder="z.B. Instagram, WhatsApp, com.whatsapp"
            ...
            <Button type="submit" disabled={isLoading} className="w-full">
              {isLoading ? (
                <>
                  ...
                  Analysiere Play Store...
                </>
              ) : (
                <>
                  <Play className="mr-2 h-4 w-4" />
                  Play Store Analyse starten
                </>
              )}
            </Button>
          </form>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{results.content_rating}</span>
                      </div>
                    </div>
                  </div>
                </div>
